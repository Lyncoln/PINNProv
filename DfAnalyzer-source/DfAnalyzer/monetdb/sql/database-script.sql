...
CREATE SEQUENCE "machine_id_seq" as integer START WITH 1;
...
CREATE TABLE machine(
	id INTEGER DEFAULT NEXT VALUE FOR "machine_id_seq" NOT NULL,
...
CREATE TABLE topology(
	id INTEGER DEFAULT NEXT VALUE FOR "topology_id_seq" NOT NULL,
...
CREATE TABLE trial(
	id INTEGER DEFAULT NEXT VALUE FOR "trial_id_seq" NOT NULL,
	tag VARCHAR(50) NOT NULL,
	df_id INTEGER NOT NULL,
	PRIMARY KEY ("id"),
	FOREIGN KEY ("df_id") REFERENCES dataflow("id") ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE task(
	...
	FOREIGN KEY ("trial_id") REFERENCES trial("tag") ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY ("dt_id") REFERENCES data_transformation("id") ON DELETE CASCADE ON UPDATE CASCADE
);
...
-- DROP FUNCTION insertMachine;
CREATE FUNCTION insertMachine (vtype VARCHAR(50), vmemory INTEGER, vprocessor VARCHAR(50))
RETURNS INTEGER
BEGIN
	DECLARE vid INTEGER;
    SELECT id INTO vid FROM machine WHERE type=vtype AND memory=vmemory AND processor=vprocessor;
    IF(vid IS NULL) THEN
    	SELECT NEXT VALUE FOR "machine_id_seq" into vid;
    	INSERT INTO machine(id,type,memory,processor) VALUES (vid,vtype,vmemory,vprocessor);
	END IF;
	RETURN vid;
END;
...
-- DROP FUNCTION insertTopology;
CREATE FUNCTION insertTopology (vactfunc VARCHAR(50), vdropout INTEGER, vdf_id INTEGER)
RETURNS INTEGER
BEGIN
	DECLARE topology_id INTEGER;
    SELECT id INTO topology_id FROM topology WHERE df_id = vdf_id AND activation_function = vactfunc AND dropout = vdropout;
    IF(topology_id IS NULL) THEN
    	#SELECT NEXT VALUE FOR "dt_id_seq" into vdt_id;
    	INSERT INTO topology(activation_function,dropout,df_id) VALUES (vactfunc,vdropout,vdf_id);
	END IF;
	RETURN SELECT id FROM topology WHERE df_id = vdf_id AND activation_function = vactfunc AND dropout = vdropout;
END;
...
CREATE FUNCTION insertTrial (etag VARCHAR(50),edf_id INTEGER)
RETURNS INTEGER
BEGIN
	DECLARE vid INTEGER;
    SELECT id INTO vid FROM trial WHERE tag=etag;
    IF(vid IS NULL) THEN	
    	INSERT INTO trial(tag,df_id) VALUES (etag,edf_id);
    END IF;
	RETURN SELECT dfe.id FROM trial dfe WHERE dfe.id=get_value_for('public', 'trial_id_seq')-1;
END;
...
CREATE FUNCTION insertTask (videntifier INTEGER, vdf_tag VARCHAR(50), vdt_tag VARCHAR(50), vstatus VARCHAR(10), vworkspace VARCHAR(500), 
	vcomputing_resource VARCHAR(100), voutput_msg TEXT, verror_msg TEXT, vtrial VARCHAR(50))
RETURNS INTEGER
BEGIN
	DECLARE vid INTEGER;
	...
	FROM dataflow df, data_transformation dt, trial dfe, dataflow_version as dfv
	WHERE df.id = dt.df_id AND dfv.df_id = df.id AND dfe.df_id = df.id AND df.tag = vdf_tag AND dt.tag = vdt_tag AND dfe.tag = vtrial;

	IF((vdf_version IS NOT NULL) AND (vdt_id IS NOT NULL) AND (ve_id IS NOT NULL)) THEN
		SELECT t.id, t.status INTO vid, vvstatus
		FROM task t
		WHERE t.df_version = vdf_version AND t.dt_id = vdt_id AND t.identifier = videntifier AND t.trial = vtrial;

		IF(vid IS NULL) THEN
		    #SELECT NEXT VALUE FOR "task_id_seq" into vid;
		    INSERT INTO task(identifier,df_version,trial_id,dt_id,status,workspace,computing_resource,output_msg,error_msg) 
		    VALUES (videntifier,vdf_version,vtrial,vdt_id,vstatus,vworkspace,vcomputing_resource,voutput_msg,verror_msg);
	    ELSE
	    	UPDATE task
	    	SET status = vstatus, output_msg = voutput_msg, error_msg = verror_msg
	    	WHERE identifier = videntifier AND df_version = vdf_version AND dt_id = vdt_id AND trial_id = vtrial;
		END IF;
    END IF;
	RETURN SELECT t.id FROM task t WHERE t.df_version = vdf_version AND t.dt_id = vdt_id AND t.identifier = videntifier AND t.trial_id = vtrial;
END;
